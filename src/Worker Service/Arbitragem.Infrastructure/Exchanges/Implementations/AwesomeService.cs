...
using ArbitraX.Infrastructure.Exchanges.Response;
using Microsoft.Extensions.Configuration;
using Newtonsoft.Json;

namespace ArbitraX.Infrastructure.Exchanges.Implementations;
        ...
        try
        {
            using (var client = new HttpClient())
            {
                ...
                if (usdPriceResponse != null && usdPriceResponse.USDBRL != null)
                {
                    ...
                    if (double.TryParse(usdPriceResponse.USDBRL.bid, out double bidValue))
                    {
                        return bidValue;
                    }
                }
