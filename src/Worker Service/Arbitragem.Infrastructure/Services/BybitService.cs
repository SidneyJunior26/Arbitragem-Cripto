...
public class BybitService : ExchangeServiceBase
{
    private double? _usdPrice;

    protected override string BuildRequestUri(int limit, Crypto crypto, List<Exchange> exchanges)
    {
        var bybit = exchanges.SingleOrDefault(e => e.Name == "Bybit");

        var endpoint = $"{bybit.ApiUrl}/market/orderbook";
        var queryString = $"category=spot&symbol={crypto.Symbol}USDT&type=step1&limit={limit}";
    ...
    public override List<OrderBook> ParseOrderBookResponse(string content, Crypto crypto, Exchange exchange, double? usdPrice)
    {
        ...
        if (jObject["retMsg"].ToString() != "OK")
            return new List<OrderBook>();

        var orderBooks = new List<OrderBook>();
        int index = 0;
        var total = 0.0;
        foreach (var bid in jObject["result"]["b"])
        {
        ...
        index = 0;
        total = 0.0;
        foreach (var ask in jObject["result"]["a"])
        {
